...
app.get('/api/hotels', (req, res) => {
  ...
  if (search) {
    ...
    console.log(`GET /api/hotels - Filtered by search: "${search}"`);
  }

  // Apply sorting if provided
  if (sort === 'visitors') {
    result.sort((a, b) => Number(b.visitors) - Number(a.visitors));
    console.log('GET /api/hotels - Sorted by visitors');
  }

  console.log(`GET /api/hotels - Returning ${result.length} hotels`);
  res.json(result);
});

app.get('/api/hotels/stats/total-rooms', (req, res) => {
  const totalRooms = hotels.reduce((sum, hotel) => {
    return sum + Number(hotel.rooms);
  }, 0);
  console.log(
    'GET /api/hotels/stats/total-rooms - Returning total rooms:',
    totalRooms
  );
  res.json({ totalRooms });
});
...
app.listen(PORT, () => {
  ...
  console.log(
    'Test it by opening http://localhost:3001/api/hotels in your browser.'
  );
});